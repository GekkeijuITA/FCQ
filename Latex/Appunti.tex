...
L'algoritmo di Deutch-Josza segue i passaggi dell'algoritmo di Deutch, in questo caso una funzione è bilancata se la metà degli output è uguale ma diversa dall'altra metà di output. Lo stato iniziale è
\begin{equation*}
    \ket{\psi_{0}} = \ket{0}^{\otimes n}\ket{-}
\end{equation*}
Vengono applicate $n$ porte di Hadarmard ai primi $n$ qubit. In questo modo otteniamo la sovrapposizione di tutte le stringhe di bit con gli interi da 0 a $N-1$. Lo stato diviene
\begin{equation*}
    \ket{\psi_{1}} = H^{\otimes n}\ket{0}^{\otimes n}\ket{-} = \left(\frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{n}}\ket{x}\right)\ket{-}
\end{equation*} 
A questo punto a questo stato viene applicato l'operatore $U_{f}$ e quindi
\begin{equation*}
    \ket{\psi_{2}}=U_{f}\ket{\psi_{1}} = \frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{n}}(-1)^{f(x)}\ket{x}
\end{equation*}
Successivamente si applicano $n$ porte di Hadarmard ai primi $n$ qubit. Per capire come queste agiscono, è utile considerare il singolo qubit $\ket{x_{i}}$ con $x_{i}=\{0,1\}$. Conosciamo il risultato dal calcolo diretto ma è utile scriverlo in maniera compatta come
\begin{equation*}
    H\ket{x_{i}} = \frac{1}{\sqrt{2}}\left(\ket{0}+(-1)^{x_{i}}\ket{1}\right)
\end{equation*}
Estendendo questo ragionamento a tutti gli $n$ qubit, otteniamo
\begin{equation*}
    H^{\otimes n}\ket{x} = \frac{1}{\sqrt{N}}\sum_{z\in\{0,1\}^{n}}(-1)^{x_{1}z_{1}+x_{2}z_{2}+\ldots+x_{n}z_{n}}\ket{z}
\end{equation*}
Che può essere riscritta in maniera compatta come:
\begin{equation*}
    H^{\otimes n}\ket{x} = \frac{1}{\sqrt{N}}\sum_{z\in\{0,1\}^{n}}(-1)^{x\cdot z}\ket{z}
\end{equation*}
Ora riscriviamo lo stato $\ket{\psi}$
\begin{equation*}
    \begin{split}
        \ket{\psi_{3}} &= H^{\otimes n}\ket{\psi_{2}}\\
        &= \frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{n}}(-1)^{f(x)}H^{\otimes n}\ket{x} \\
        &= \sum_{x\in\{0,1\}^{n}}\left(\frac{1}{N}\sum_{z\in\{0,1\}^{n}}(-1)^{f(x)}(-1)^{x\cdot z}\right)\ket{z}
    \end{split}
\end{equation*}
...
\begin{equation*}
    \frac{1}{N}\sum_{x\text{ costante}} (-1)^{\bar{f}} = \frac{1}{N}\cdot \pm N = \pm 1 
\end{equation*}
...
\begin{equation*}
    \frac{1}{N}\sum_{x\in\{0,1\}^{n}} (-1)^{f(x)} = \frac{1}{N}\left[\left(\sum_{f(x)=0}1\right)+\left(\sum_{f(x)=1}-1\right)\right] = \frac{1}{N}\left(\frac{N}{2}-\frac{N}{2}\right) = 0
\end{equation*}
...
\begin{equation*}
    \begin{split}
        \ket{\psi_{2}} &= H^{\otimes n}\ket{\psi_{1}}\\
        &= \frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{\otimes n}} \ket{x}\ket{-}\\
        &\xrightarrow{U_{f}} \frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{\otimes n}} (-1)^{f(x)}\ket{x}\ket{-}\\
        &\xrightarrow{H^{\otimes n}} \frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{\otimes n}} (-1)^{x\cdot a}H^{\otimes n}\ket{x}\\
        &= \frac{1}{N}\sum_{x\in\{0,1\}^{\otimes n}} (-1)^{x\cdot a} \sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x\cdot z}\ket{z}\\
        &= \frac{1}{N}\sum_{x\in\{0,1\}^{\otimes n}}\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x\cdot a}(-1)^{x\cdot z}\ket{z}\\
        &= \frac{1}{N}\sum_{x\in\{0,1\}^{\otimes n}}\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x\cdot a + x\cdot z}\ket{z}\\
        &= \frac{1}{N}\sum_{x\in\{0,1\}^{\otimes n}}\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x\cdot (a\oplus z)}\ket{z}\\
        &= \frac{1}{N}\sum_{x\in\{0,1\}^{\otimes n}}\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x\cdot (a\oplus z)}\ket{z}\\
    \end{split}
\end{equation*}
Se $z=a$ allora $a\oplus z = 0$ quindi sarà $(-1)^{0} = 1$ e avremo che
\begin{equation*}
    \frac{1}{N}\sum_{x\in\{0,1\}^{\otimes n}} 1 = \frac{1}{N}\cdot N = 1
\end{equation*}
Questa è la probabilità di trovare $a$ una volta "lanciato" l'algoritmo.
...
\begin{equation*}
    \ket{\psi_{1}} = H^{\otimes n}\ket{\psi_{0}} =\frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{\otimes n}}\ket{x}\ket{0}
\end{equation*}
Applichiamo l'oracolo e, dato che $\ket{0\oplus f(x)} = \ket{f(x)}$
\begin{equation*}
    \ket{\psi_{1}} \xrightarrow{O} \ket{\psi_{2}} = \frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{\otimes n}} \ket{x}\ket{f(x)}
\end{equation*}
Dato che per ogni $x$ esiste un $x\oplus a$ tale che $f(x)=f(x\oplus a)$, possiamo scrivere
\begin{equation*}
    \ket{\psi_{2}} = \frac{1}{\sqrt{\frac{N}{2}}} \sum_{x\in\{0,1\}^{\otimes n}}\frac{\ket{x}+\ket{x\oplus a}}{\sqrt{2}}\ket{f(x)}
\end{equation*}
...
\begin{equation*}
    \begin{split}
        \frac{\ket{x_{0}}+\ket{x_{0}\oplus a}}{\sqrt{2}} &\xrightarrow{H^{\otimes n}} \frac{1}{\sqrt{2}}\left(\frac{1}{\sqrt{N}}\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x_{0}\cdot z}\ket{z} + \frac{1}{\sqrt{N}}\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{(x_{0}\oplus a)\cdot z}\ket{z}\right)\\
        &= \frac{1}{\sqrt{2}}\left(\frac{1}{\sqrt{N}}\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x_{0}\cdot z}\ket{z} + \frac{1}{\sqrt{N}}\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x_{0}\cdot z + a\cdot z}\ket{z}\right) \\
        &= \frac{1}{\sqrt{2N}}\left(\sum_{z\in\{0,1\}^{\otimes n}}(-1)^{x_{0}\cdot z}\left(1+(-1)^{a\cdot z}\right)\ket{z}\right)
    \end{split}
\end{equation*}
...
\begin{equation*}
    \ket{\psi} = \frac{1}{\sqrt{N}}\sum_{x\in\{0,1\}^{\otimes n}}\ket{x}
\end{equation*}
